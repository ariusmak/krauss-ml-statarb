"""
Run Phase 2 extension: train RF, XGB, multitask DNN across all 23 study periods,
generate P_hat and U_hat predictions, compute ENS1 and score families.

Usage:
    python scripts/run_phase2.py
    python scripts/run_phase2.py --model rf          # RF only
    python scripts/run_phase2.py --model xgb         # XGB only
    python scripts/run_phase2.py --model dnn         # DNN only
    python scripts/run_phase2.py --periods 0 1 2     # specific periods only

Outputs:
    data/models_p2/period_{id}/rf_cls.pkl     — trained RF classifier
    data/models_p2/period_{id}/rf_reg.pkl     — trained RF regressor
    data/models_p2/period_{id}/xgb_cls.json   — trained XGB classifier
    data/models_p2/period_{id}/xgb_reg.json   — trained XGB regressor
    data/models_p2/period_{id}/mt_dnn.pt      — trained multitask DNN
    data/models_p2/period_{id}/meta.json      — period metadata
    data/processed/predictions_phase2.parquet  — all predictions + score families
"""
...
from krauss.models.ensembles_phase2 import (
    ens1_p_hat, ens1_u_hat, compute_score_families,
)

ROOT = Path(__file__).resolve().parent.parent
PROCESSED = ROOT / "data" / "processed"
MODELS = ROOT / "data" / "models_p2"
...
def run_period(
    ...
    """Train, predict, and save extension models for one study period."""
    t0 = time.time()
...
pandas_out = pd.DataFrame({{'p': p_trade, 'u': u_trade}})
pandas_out.to_parquet({str(model_dir / '_rf_trade.parquet')!r}, index=False)
pandas_out_tr = pd.DataFrame({{'p': p_train, 'u': u_train}})
pandas_out_tr.to_parquet({str(model_dir / '_rf_train.parquet')!r}, index=False)
""", model_dir)
        else:
            _run_model_subprocess("RF-ext", f"""
import sys; sys.path.insert(0, {src_path!r})
import joblib, pandas as pd
...
p_trade = clf.predict_proba(tdp[FEATURE_COLS])[:, 1]
u_trade = reg.predict(tdp[FEATURE_COLS])
p_train = clf.predict_proba(tp[FEATURE_COLS])[:, 1]
u_train = reg.predict(tp[FEATURE_COLS])
...
""", model_dir)
            ...
            _run_model_subprocess("XGB-ext", f"""
import sys; sys.path.insert(0, {src_path!r})
import pandas as pd
...
""", model_dir)
    ...
    if run_rf and run_xgb and run_dnn:
        # ENS1: average across model families
        ...
        # Score families for each model + ENS1
        ...
        print(f"    ENS1 + score families computed")
...
def main():
    ...
    print("=" * 60)
    print("PHASE 2 — Extension Model Training & Prediction")
    print("=" * 60)
    ...
    # Build study periods (same structure as Phase 1)
    ...
    for pid in tqdm(remaining, desc="Study periods", unit="period"):
        ...
        all_so_far.to_parquet(PROCESSED / "predictions_phase2.parquet", index=False)

    # Final consolidated save
    predictions = pd.concat(all_results, ignore_index=True)
    out_path = PROCESSED / "predictions_phase2.parquet"
    ...
    print(f"\nModels saved to data/models_p2/:")
    for pid in period_ids:
