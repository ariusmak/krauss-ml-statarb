...
def validate_model_returns(
    ...
    *,
    ...
    if check_sources:
        expected_sources = metadata.get("source_snapshot")
        current_sources = _source_snapshot()
        expected_has_sources = any(
            snapshot.get("exists") for snapshot in (expected_sources or {}).values()
        )
        current_has_sources = any(
            snapshot.get("exists") for snapshot in current_sources.values()
        )
        if (
            expected_has_sources
            and current_has_sources
