...
st.set_page_config(page_title="System Architecture", page_icon=":bar_chart:",
                   layout="wide")
...
model_meta = load_model_returns_metadata() or {}
...
st.caption(
    "From raw vendor panels to validated `model_returns.parquet`. This is "
    "the production-system proof behind the research dashboard."
)
...
- **Datastream US** — independent vendor used for the 2015-live out-of-sample
  extension. Coverage {meta['eras']['extension']['start']} to
  {model_meta.get('signal_date_max', meta['eras']['extension']['end'])}.
  Source: {meta['data_sources']['datastream']}; the current live tail is
  gap-filled from yfinance after the Datastream panel endpoint.
- **S&P 500 index** — daily close used to derive VIX-style volatility
  regimes on the Results & Risk Diagnostics page. Source:
  {meta['data_sources']['sp500_index']}.
        """
    )
...
| P-gate(c) | confident P_hat rows, ranked by U_hat | Gate uncertain rows |
        """
    )
...
- *Gross / no transaction cost* (`no_cost`) — gross returns, for like-for-like
  comparison with the paper's pre-cost tables.
- *Post-cost: 5 bps per half-turn* (`5bps_half_turn`) — the realistic cost
  assumption used in every post-cost claim.
        """
    )
