...
Five scoring schemes, plus the interactive directional-disagreement scatter
widget and a rolling disagreement-rate chart that overlays the CRSP and the
2015-2025 extension eras.
"""
...
from lib.data import (  # noqa: E402
    ...
    load_disagreement_panel, load_summary_table,
)
...
st.caption(
    "Five ways to combine direction (P̂) and magnitude (Û) into a single "
    "per-stock score. All schemes run on ENS1 by default. P-gate is shown "
    "as a family because the gate threshold behaves non-linearly — 0.03 "
    "gives a meaningful subset while 0.05 is much more selective."
)

# --- Prose description + quick-stat cards -----------------------------------
st.subheader("The five schemes at a glance")
...
cards = [
    ...
    ("Z-comp",
     "0.5·z(P̂) + 0.5·z(Û), z-scored per-day. The strongest unconditional "
     "composite — a modest but consistent lift over P-only."),
    ("Product",
...
daily_rate["rolling_252"] = (
    daily_rate.groupby("era")["disagree_rate"]
...
fig2 = go.Figure()
for era, grp in daily_rate.groupby("era"):
