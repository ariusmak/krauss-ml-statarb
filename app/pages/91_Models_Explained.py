...
st.caption(
    ":books: **Appendix** — model detail supporting the Methodology & "
    "Extensions page."
)
...
HYPERPARAMS = {
    ...
    "MT-DNN": dict(
        ...
        rows=[
            ...
            ("trained on",
              "Datastream Phase 2 rolling windows, including the live period-33 model"),
        ],
    ),
    "RF cls+reg": dict(
...
Weaknesses. The single shared trunk can under-fit the regression head
relative to a dedicated regressor, so schemes that lean heavily on U can be
sensitive to the DNN's compressed magnitude output. In the app, the Phase-2
MT-DNN predictions are displayed under the familiar **DNN** model label so
the ensemble notation stays consistent across eras.
"""
)
_hp_table("MT-DNN")
st.info(
    "There is no separate MT-DNN row in the results matrix. Its P̂ and Û "
    "outputs are the Phase-2 DNN member used inside the `DNN` and `ENS1` rows; "
    "`data/models_p2_ds/period_33/mt_dnn.pt` is the frozen live-tail artifact."
)
...
st.caption(
    ...
    "file and line range. Where a model has an era-specific interpretation "
    "(for example, Phase-2 DNN means the multi-task DNN head), the page says "
    "so explicitly rather than inferring a plausible default."
)
